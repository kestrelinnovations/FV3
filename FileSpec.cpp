...
#else
		_tcscpy_s(tszPath, ptsz = _tempnam(tszPath, _T("~")));
#endif
		SetFullSpec(LPCTSTR(tszPath));
		free(ptsz);
		break;
...
void CFileSpec::SetFileName(LPCTSTR szSpec)
{
	...
	_tsplitpath_s(szSpec, NULL, 0, NULL, 0, tszFilename, sizeof(tszFilename), tszExtension, sizeof(tszExtension));
...
void CFileSpec::SetFullSpec(LPCTSTR szSpec)
{
	...
	if (IsUNCPath(szSpec))
	{
		...
		_tsplitpath_s(ptsz, tszDrive, tszPath, tszFilename, tszExtension);
		m_csPath = tszPath;
		m_csFilename = tszFilename;
		m_csExtension = tszExtension;
	}
			...
			do
			{
				...
				if (res == ERROR_SUCCESS || res == ERROR_MORE_DATA)
				{
					...
					if (iBestMatch)
					{
						...
						csTemp = GetFolder();
						csTemp = csTemp.Right(csTemp.GetLength() - iBestMatch ); // + 1); MATT_FIX
