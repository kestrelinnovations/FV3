...
static void GetClientRectSB(CWnd* pWnd, CSize itemSize, CRect& clientRect)
{
    ASSERT( pWnd != NULL );
    ...
    if ( winRect.bottom >= (clientRect.bottom + cySB) )
	 ...
	 // We need to adjust client area size when scroll bars are present.
	 if ( itemSize.cy > clientRect.bottom )
	 {
		 clientRect.right -= cxSB;
	 }

	 if ( itemSize.cx > clientRect.right )
	 {
		 clientRect.bottom -= cySB;
	 }
}
...
void CScrollHelper::UpdateScrollInfo()
{
    if ( m_attachWnd == NULL )
	 ...
	 // expect.
	 CRect rect;
	 GetClientRectSB(m_attachWnd, m_displaySize, rect);
