...
// IconItem.cpp: implementation of the CIconItem class.
//
//////////////////////////////////////////////////////////////////////
...
CIconItem::CIconItem(C_overlay *overlay) : C_icon(overlay)
{
	m_bbox.Zero();
	m_sourceID.m_lIdentity = -1;
	m_sourceID.m_lServer = -1;
	m_mapTypeID	= -1;
	m_flags		= 0;
}
...
void CIconItem::Draw(MapProj* map, CDC* pDC)
{
	COLORREF color;
	int style;
	int weight;
	...
	if (m_flags & CV_CATALOG)
		color = (m_flags & CV_SELECTED) ? bright_yellow : yellow;
	else if (m_flags & CV_TARGET)
		color = (m_flags & CV_SELECTED) ? bright_green : green;
	else
		color = (m_flags & CV_SELECTED) ? bright_red : red;

	//	Determine style: back hatch if overlapped (even/odd flag)
	style = (m_flags & CV_CATALOG) ? HS_VERTICAL : ((m_flags & CV_ODD) ? HS_BDIAGONAL : HS_FDIAGONAL);
   ...
   // Handle special cases for tiny rectangles
	...
	{
      int drawBrushStyle = cat_getDrawBrushStyle();
      if (drawBrushStyle == DRAWBRUSHSTYLE_HOLLOW)// draw using minimal tools
      {
         CBrush brush;
      ...
      else if (drawBrushStyle == DRAWBRUSHSTYLE_HATCH)	// draw using hatch brush tools (for printing)
      {
      ...
      else if (drawBrushStyle == DRAWBRUSHSTYLE_TRANSPARENT)	// draw using transparent tools
      {
         CBrush brush(color);
		   CPen pen(PS_SOLID, weight, color);
      ...
      else if (drawBrushStyle == DRAWBRUSHSTYLE_PATTERN)	// draw using pattern brush tools
      {
         int idMask;
         int idColor;
         ...
         if (m_flags & CV_ODD)
         {
	         //	Determine color: should be green if on target device, bright if selected
	         if (m_flags & CV_CATALOG)
		         idColor = (m_flags & CV_SELECTED) ? IDB_MDM_CATSEL1 : IDB_MDM_CAT1;
	         else if (m_flags & CV_TARGET)
		         idColor = (m_flags & CV_SELECTED) ? IDB_MDM_TGTSEL1 : IDB_MDM_TGT1;
	         else
		         idColor = (m_flags & CV_SELECTED) ? IDB_MDM_SRCSEL1 : IDB_MDM_SRC1;
         ...
         else
         {
	         //	Determine color: should be green if on target device, bright if selected
	         if (m_flags & CV_CATALOG)
		         idColor = (m_flags & CV_SELECTED) ? IDB_MDM_CATSEL2 : IDB_MDM_CAT2;
	         else if (m_flags & CV_TARGET)
		         idColor = (m_flags & CV_SELECTED) ? IDB_MDM_TGTSEL2 : IDB_MDM_TGT2;
	         else
		         idColor = (m_flags & CV_SELECTED) ? IDB_MDM_SRCSEL2 : IDB_MDM_SRC2;
   ...
   // also draw extra thick hollow rectangle if highlighting a catalog icon
	if ((m_flags & CV_CATALOG) && (m_flags & CV_HIGHLIGHTED))
   {
   	weight = 4;
...
CString CIconItem::get_help_text()
{
	CString sInfo;
...
CString CIconItem::get_tool_tip()
{
	CString sInfo;
...
boolean_t CIconItem::hit_test(CPoint point)
{
	return TRUE;
}
	...
	try
	{
      ...
      switch (m_type)
      {
         ...
         case CV_SELECTED_CATALOG:
            ...
            break;
         case CV_HIGHLIGHTED_CATALOG:
         case CV_HIGHLIGHTED_SELECTED_CATALOG:
            color = bright_orange;
      	   style = HS_VERTICAL;
         	weight = 3;
         	weightframe = 5;
